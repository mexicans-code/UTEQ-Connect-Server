...
export const createSingleInvitation = async (
    ...
    const evento = await Evento.findOneAndUpdate(
        { _id: eventoId, cuposDisponibles: { $gt: 0 }, activo: true },
    ...
    if (!evento) throw new Error('No hay cupos disponibles o el evento no está activo');
...
export const findInvitationByToken = async (token: string) =>
    EventInvitation.findOne({ token }).populate('evento').populate('usuario', '-password');
...
export const findInvitationsByEvent = async (eventoId: string) =>
    EventInvitation.find({ evento: eventoId })
        .populate('usuario', '-password')
        .sort({ fechaEnvio: -1 });

export const findInvitationsByUser = async (userId: string) =>
    EventInvitation.find({ usuario: userId })
        .populate('evento')
        .sort({ fechaEnvio: -1 });
...
export const markAttendance = async (invitationId: string, asistio: boolean) => {
    const invitation = await EventInvitation.findById(invitationId);
    if (!invitation) return null;
    invitation.estadoAsistencia = asistio ? 'asistio' : 'no_asistio';
    if (asistio && !invitation.fechaUsoToken) invitation.fechaUsoToken = new Date();
    await invitation.save();
    return invitation;
};
