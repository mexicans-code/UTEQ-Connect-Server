...
export const getEspaciosSugeridos = async (req: Request, res: Response) => {
    try {
...
export const updateEspacio = async (req: Request, res: Response) => {
    try {
        ...
        if (cupos     !== undefined) updateData.cupos      = Number(cupos);
        if (planta    !== undefined) updateData.planta     = planta;
        if (descripcion !== undefined) updateData.descripcion = String(descripcion).trim();
        if (ocupado   !== undefined) updateData.ocupado    = Boolean(ocupado);

        const espacio = await espacioService.updateEspacio(req.params.id as string, updateData);
        if (!espacio) return res.status(404).json({ success: false, error: "Espacio no encontrado" });
...
export const deleteEspacio = async (req: Request, res: Response) => {
    try {
        ...
        if (espacio.image) {
            try { await cloudinary.uploader.destroy(getPublicId(espacio.image)); } catch {}
        }

        await espacioService.deleteEspacio(req.params.id as string);
...
export const uploadImage = async (req: Request, res: Response) => {
    try {
        const espacio = await espacioService.findEspacioById(req.params.id as string);
        if (!espacio) return res.status(404).json({ success: false, error: "Espacio no encontrado" });
        if (!req.file)  return res.status(400).json({ success: false, error: "No se proporcionó imagen" });
        ...
        const updated = await espacioService.updateEspacio(req.params.id as string, { image: req.file.path });
...
export const deleteImage = async (req: Request, res: Response) => {
    try {
        ...
        const updated = await espacioService.updateEspacio(req.params.id as string, { image: null } as any);
