...
export const recalcularOcupadoEspacio = async (espacioId: string) => {
  const now = new Date();
...
const checkEspacioConflict = async (eventData: any, eventId?: string) => {
  if (!eventData.espacio) return { hasConflict: false, conflicts: [] };
  ...
  if (conflicts.length > 0) {
    return {
      ...
      conflicts: conflicts.map(c => {
        const enc = c.creadoPor as IPersonalPopulated | undefined;
        return {
          titulo: c.titulo,
...
const checkEventConflict = async (eventData: any, eventId?: string) => {
  ...
  if (conflictingEvents.length === 0) return { hasConflict: false, conflicts: [] };
...
export const createEvent = async (eventData: any) => {
  try {
    ...
    if (conflictDest.hasConflict) {
      const msgs = conflictDest.conflicts.map(c =>
    ...
    if (conflictEsp.hasConflict) {
      const msgs = conflictEsp.conflicts.map(c =>
        `• "${c.titulo}" (${c.horaInicio}-${c.horaFin})`
      ).join('\n');
    ...
    if (isNaN(fechaFin.getTime()))    throw new Error('Fecha de fin inválida');
    ...
    if (eventData.espacio) await recalcularOcupadoEspacio(eventData.espacio.toString());
...
export const updateEvent = async (id: string, eventData: any) => {
  try {
    ...
    const updated = await eventPopulate(
      Event.findByIdAndUpdate(id, eventData, { new: true, runValidators: true })
    );
    ...
    if (espacioAnterior) await recalcularOcupadoEspacio(espacioAnterior);
    if (espacioNuevo && espacioNuevo !== espacioAnterior) await recalcularOcupadoEspacio(espacioNuevo);
...
export const deleteEvent = async (id: string) => {
  try {
    ...
    if (event?.espacio) await recalcularOcupadoEspacio(event.espacio.toString());
...
export const updateCuposDisponibles = async (id: string, cantidad: number) => {
  try {
    ...
    if (event.cuposDisponibles < 0) throw new Error("No hay suficientes cupos disponibles");
    if (event.cuposDisponibles > event.cupos) throw new Error("Los cupos disponibles no pueden exceder los cupos totales");
    await event.save();
    return event;
  } catch (error) {
    throw error;
...
export const deactivateExpiredEvents = async () => {
  try {
    ...
    const result = await Event.updateMany(
      { activo: true, desactivarEn: { $lte: now } },
      { $set: { activo: false } }
    );
