...
export const updateUser = async (id, userData) => {
    try {
        const user = await User.findByIdAndUpdate(id, userData, { new: true, runValidators: true }).select('-passwordHash');
        return user;
    }
    catch (error) {
        throw error;
...
export const updateUltimoLogin = async (id) => {
    try {
        const user = await User.findByIdAndUpdate(id, { ultimoLogin: new Date() }, { new: true }).select('-passwordHash');
        return user;
    }
    catch (error) {
...
export const changeUserStatus = async (id, estatus) => {
    try {
        const user = await User.findByIdAndUpdate(id, { estatus }, { new: true }).select('-passwordHash');
        return user;
    }
    catch (error) {
...
export const findUsersByRol = async (rol) => {
    try {
